...
fn make_solution_map(tree: &Tree) -> HashMap<String, Vec<Solution>> {
...
.not-done>td a:link { color: rgba(0, 0, 255, 0.382); }",
...

    {
        ...
        thead_tr.add_element("th").add_text("Rust Solutions");
    }
    ...
    for problem in problems {
        if let Some(solutions) = solution_map.get(&problem.get_id()) {
