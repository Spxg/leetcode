        ...
        for (homepage, operations) in test_cases {
            ...
            for operation in operations {
                match *operation {
                    ...
                    Operation::Back(steps, expected) => {
                        assert_eq!(browser_history.back(steps), expected);
                    }
                    Operation::Forward(steps, expected) => {
                        assert_eq!(browser_history.forward(steps), expected);
                    }
                }
            }
        }
