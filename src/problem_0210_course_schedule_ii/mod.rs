...
mod tests {
    ...
    pub fn run<S: Solution>() {
        ...
        for ((num_courses, prerequisites), can_finish) in test_cases {
            let result = S::find_order(num_courses, prerequisites.iter().map(Vec::from).collect());

            if can_finish {
                assert_eq!(
                    result.iter().copied().collect::<HashSet<_>>().len(),
                    num_courses as _
                );
                ...
                for &[course, prerequisite] in prerequisites {
                    assert!(
                        course_indices[prerequisite as usize] < course_indices[course as usize]
                    );
                }
            } else {
